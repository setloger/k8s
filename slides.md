...
# 9. Обновление кластера

---

## Почему нельзя откладывать

- 3 минорные версии в год
- Каждая поддерживается ~**14 месяцев**
- Отставание на 2+ версии = без security-патчей
- В банковском секторе это неприемлемо

**Поддерживаемые сейчас:** 1.34, 1.35, 1.36

---

## Как устроено у нас: Ansible

Точка входа: `play-upgrade-kube.yml`
...
| `kube_infra` | Инфраструктурные |

`serial: 1` + `any_errors_fatal: true`

---

## Идемпотентность встроена в плейбук

Перед обновлением каждой ноды роль проверяет:

```yaml
# 1. kubelet в состоянии KubeletReady?
assert:
  that:
    - k8s_node_status == 'KubeletReady'

# 2. Нода ещё не на целевой версии?
when:
  - 'k8s_node_version != "v" + kube_version'
```

Плейбук можно запускать повторно без риска

---

## Стратегия: строгий порядок

1. 🔒 **Бэкап etcd** — перед запуском плейбука, без исключений
2. ⬆️ **Control plane первым** — `kube_master`, `serial: 1`
3. 🔄 **Workers и инфра после** — `kube_nodes`, `kube_ingress`, `kube_infra`
4. 🚫 **Нельзя прыгать через версию** — плейбук запускается трижды

---

## Ansible: обновление control plane

```yaml
# 1. Drain (k8s_upgrade_master_drain_enabled)
kubectl drain {{ inventory_hostname }} --ignore-daemonsets

# 2. yum versionlock delete + yum install
yum install kubeadm-{{ yum_kube_version }} kubelet-{{ yum_kube_version }}
yum versionlock kubeadm kubelet kubectl

# 3. kubeadm upgrade apply (retries: 3)
kubeadm upgrade apply -y {{ kube_version }} \
  --ignore-preflight-errors=KubernetesVersion --force

# 4. systemctl restart kubelet
# 5. uncordon (if not pre-cordoned)
# 6. Авто: k8s-cis-settings.yml (CIS Benchmark)
```

---

## Ansible: обновление worker ноды

```yaml
# 1. Drain — delegate_to: первый мастер
kubectl drain {{ inventory_hostname }} \
  --ignore-daemonsets --delete-emptydir-data

# 2. yum versionlock delete + yum install — на ноде
yum install kubeadm-{{ yum_kube_version }} kubelet-{{ yum_kube_version }}
yum versionlock kubeadm kubelet kubectl
...
# 5. Uncordon — delegate_to: первый мастер
kubectl uncordon {{ inventory_hostname }}
```

---

## Hard Requirements с v1.35

**cgroup v1 — удалён** 🔴
```bash
stat -fc %T /sys/fs/cgroup/
# cgroup2fs — OK
# tmpfs — нужна миграция
```

**containerd 1.x — EOL** 🔴
Необходим containerd 2.x
Roль `ansible-cri-containerd` — обновить **до** запуска плейбука

---

## Типичные грабли on-prem

- **PDB мешает drain** — `any_errors_fatal` остановит весь плейбук. `--disable-eviction` игнорирует PDB, осторожно
- **DaemonSet поды** — `--ignore-daemonsets` уже зашит в плейбуке
